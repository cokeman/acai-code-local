...
class QuantumAPI {
    ...
    static function getChatsNumsBySearchTerm($searchTerm, $status, $domain){
        ...
        $contacts = CocoDB::get("contactos","(nombre LIKE '%".mysql_real_escape_string($searchTerm)."%' or telefono LIKE '%".mysql_real_escape_string($searchTerm)."%') and chat_conf_num = ".intval($chat_conf_num),"num desc",null,["ignoreSchemas" => true]);
        $contacts = array_map(function($r){ return $r["num"];},$contacts);

        $where1 = "website = '".mysql_real_escape_string($domain)."' and status = '".mysql_real_escape_string($status)."' and uuid not like 'test-widget%' and trashed = 0";
        if (@$contacts){
            $where = $where1 . " and (contactNum IN (".join(",",$contacts).") OR uuid LIKE '%".mysql_real_escape_string($searchTerm)."%')";
            // OBTENGO TODOS LOS CHATS QUE CUMPLEN EL DOMINIO Y ESTADO DE LOS CONTACTOS Y UUID ENCONTRADOS 
            $chatNums = mysql_query_fetch_all_assoc("SELECT num FROM cocochats WHERE ".$where);
            $chatNums = array_map(function($r){ return $r["num"];},$chatNums);
        }else{
            $where = $where1;
            $chatNums = [];
        }

        // OBTENGO TODOS LOS CHATS POSIBLES PARA ESE DOMINIO Y ESTADO
        $allChatNums = mysql_query_fetch_all_assoc("SELECT DISTINCT(num) FROM cocochats WHERE ".$where);
        $allChatNums = array_map(function($r){ return $r["num"];},$allChatNums);
        if (@$allChatNums){
            $messages = mysql_query_fetch_all_assoc("SELECT chat_id FROM cocochats_mensajes WHERE mensaje LIKE '%".mysql_real_escape_string($searchTerm)."%' and chat_id IN (".join(",",$allChatNums).")");
            if ($messages){
                $chatNumsFromMessages = array_map(function($r){ return $r["chat_id"];},$messages);
                $chatNums = array_merge($chatNums, $chatNumsFromMessages);
            }
        }
