...
class QuantumAPI_OLD {
    static function getChat($request){
        ...
        if (!$chatResult){
            ...
            // Guardar whatsappPhoneId en whatsappPhoneId
            if (@$request["whatsappPhoneId"]) {
                $data["whatsappPhoneId"] = $request["whatsappPhoneId"];
            }
            ...
            $chatInfoUpdate = CocoDB::updateRecords("cocochats",["lastConnectionDate" => date("Y-m-d H:i:s")],"num=".$chatResult[0]["num"],null,["prefix" => "","ignoreSchema" => true]);
        }
    ...
    static function getEmbeddingsCSV($request){
        ...
        $records = CocoDB::get("cocochats_training_embeddings","training_data_num IN (".join(",",$numsTraining).")","num asc",null,["prefix" => "","ignoreSchemas" => true]);
        

        $records = array_map(function($record) {
            return array_intersect_key($record, array_flip(["num","text", "n_tokens", "embeddings"]));
        }, $records);
    ...
    public static function getChatByContactAndPhoneId($paramsOrContactId = null, $whatsappPhoneId = null, $domain = null) {
        // Soporta: (1) objeto/array JSON o (2) 3 argumentos posicionales
        if (is_array($paramsOrContactId) || is_object($paramsOrContactId)) {
            $p = is_object($paramsOrContactId) ? (array)$paramsOrContactId : $paramsOrContactId;
    
            // Desenrollar si viene como { endpoint, data }
            if (isset($p['endpoint']) && isset($p['data'])) {
                $p = $p['data'];
            }
    
            // Asociativo
            $contactId       = $p['contact_id'] ?? ($p['contactId'] ?? null);
            $whatsappPhoneId = $p['whatsapp_phone_id'] ?? ($p['whatsappPhoneId'] ?? null);
            $domain          = $p['domain'] ?? null;
    
            // Posicional [contactId, whatsappPhoneId, domain]
            if ($contactId === null && isset($p[0])) $contactId = $p[0];
            if ($whatsappPhoneId === null && isset($p[1])) $whatsappPhoneId = $p[1];
            if ($domain === null && isset($p[2])) $domain = $p[2];
        } else {
            // Firma posicional tradicional
            $contactId = $paramsOrContactId;
        }
    
        // Saneado
        $contactId = intval($contactId);
        $phoneId   = mysql_real_escape_string((string)$whatsappPhoneId);
        $domainEsc = mysql_real_escape_string((string)$domain);
    
        if (!$contactId || !$phoneId || !$domainEsc) {
            return ['success' => false, 'data' => null, 'error' => 'Parámetros inválidos'];
        }
    
        // Consulta principal (camelCase en BD)
        $query = "
            SELECT uuid, num AS chat_num, status
            FROM cocochats
            WHERE website = '{$domainEsc}'
              AND trashed = 0
              AND uuid NOT LIKE 'test-widget%'
              AND contactNum = {$contactId}
              AND whatsappPhoneId = '{$phoneId}'
            ORDER BY num DESC
            LIMIT 1
        ";
        $row = mysql_query_fetch_all_assoc($query);
        if ($row && count($row) > 0) {
            return ['success' => true, 'data' => $row[0]];
        }
    
        // Fallback en metadatos (camelCase)
        $queryMeta = "
            SELECT c.uuid, c.num AS chat_num, c.status
            FROM cocochats c
            JOIN cocochats_meta m ON m.chat_id = c.num
            WHERE c.website = '{$domainEsc}'
              AND c.trashed = 0
              AND c.uuid NOT LIKE 'test-widget%'
              AND c.contactNum = {$contactId}
              AND m.meta_key = 'whatsappPhoneId'
              AND m.meta_value = '{$phoneId}'
            ORDER BY c.num DESC
            LIMIT 1
        ";
        $row = mysql_query_fetch_all_assoc($queryMeta);
        if ($row && count($row) > 0) {
            return ['success' => true, 'data' => $row[0]];
        }
...
/*
    // ========================================
    // NUEVA SECCIÓN: CONTADORES AGREGADOS
    // ========================================
    $aggregates = null;
    
    if (@$request["notReaded"] && @$request["userHash"]) {
        $aggregates = self::getUnreadMessageAggregates($request["domain"], $request["userHash"], $request);
    }
...
*/
// ========================================
// NUEVA FUNCIÓN AUXILIAR PARA CONTADORES
// ========================================
static function getUnreadMessageAggregates($domain, $userHash, $request = []) {
    global $TABLE_PREFIX;
    
    $options = ["prefix" => "", "ignoreSchemas" => true];
    
    // Base WHERE clause para filtrar chats del dominio
    $baseWhere = "website = '".mysql_real_escape_string($domain)."' and uuid not like 'test-widget%' and trashed = 0";
    
    // Aplicar filtros adicionales si existen
    if (@$request["chatType"]) {
        $baseWhere .= " and chatType = '".mysql_real_escape_string($request["chatType"])."'";
    } else {
        $baseWhere .= " and (chatType = 'default' or chatType = 'internal')";
    }
    
    if (@$request["status"]) {
        $baseWhere .= " and status = '".mysql_real_escape_string($request["status"])."'";
    } else {
        $baseWhere .= " and status = 'Abierta'";
    }
    
    if (@$request["dates"]) {
        $dates = $request["dates"];
        if (@$dates[0] && @$dates[1]) {
            $baseWhere .= " and lastMessageDate >= '".mysql_real_escape_string(explode("T",$dates[0])[0])." 00:00:00'";
            $baseWhere .= " and lastMessageDate <= '".mysql_real_escape_string(explode("T",$dates[1])[0])." 23:59:59'";
        }
    }

    // 1. CONTADOR: Chats con último mensaje sin leer
    $chatsWithUnreadWhere = $baseWhere . " and (SELECT count(*) FROM cocochats_mensajes ch WHERE ch.chat_id = cocochats.num and ch.leido_por NOT LIKE '%\t".$userHash."\t%' ORDER BY ch.num DESC LIMIT 1) > 0";
    $chatsWithUnread = CocoDB::get("cocochats", $chatsWithUnreadWhere, "", null, $options);
    $chatsWithUnreadCount = count($chatsWithUnread ?: []);

    // 2. CONTADOR: Conversaciones con mensajes sin leer
    $conversationsWithUnreadWhere = $baseWhere . " and (SELECT count(*) FROM cocochats_mensajes ch WHERE ch.chat_id = cocochats.num and ch.leido_por NOT LIKE '%\t".$userHash."\t%') > 0";
    $conversationsWithUnread = CocoDB::get("cocochats", $conversationsWithUnreadWhere, "", null, $options);
    $conversationsWithUnreadCount = count($conversationsWithUnread ?: []);

    // 3. CONTADOR: Total de mensajes sin leer
    // Obtener todos los chats que cumplen los criterios
    $allChats = CocoDB::get("cocochats", $baseWhere, "", null, $options);
    $totalUnreadMessages = 0;
    
    if ($allChats) {
        foreach ($allChats as $chat) {
            // Contar mensajes sin leer para este chat específico
            $unreadMessagesForChat = CocoDB::get(
                "cocochats_mensajes", 
                "chat_id = " . intval($chat["num"]) . " and leido_por NOT LIKE '%\t".$userHash."\t%'", 
                "", 
                null, 
                $options
            );
            $totalUnreadMessages += count($unreadMessagesForChat ?: []);
        }
    }

    return [
        "chatsWithUnread" => $chatsWithUnreadCount,
        "conversationsWithUnread" => $conversationsWithUnreadCount,
    ...
    static function addMessage($request){
        ...
        if ((@$request["origin"] != "assistant" || @$_REQUEST["userData"]["forceCreate"]) && @$chatResult[0] && (@$_REQUEST["userData"]["email"] || @$chatResult[0]["channel"] == "whatsapp" || @$chatResult[0]["channel"] == "instagram" || @$chatResult[0]["channel"] == "messenger" || @$chatResult[0]["channel"] == "telegram")) {
            ...
            if ($chatResult[0]["channel"] == "whatsapp"){
                ...
                $userData["telefono"] = preg_replace("/[^0-9]/", "", @$userData["telefono"]);
    ...
    static function updateUserHashInChat($request){
        if (!@$request["oldUserHash"] || !@$request["chat_num"] || !@$request["newUserHash"]){
            throw new ApiError("oldUserHash, chat_num and newUserHash needed");
        } 

        $chatNum = @$request["chat_num"];
        $userHash = @$request["newUserHash"];
        $oldUserHash = @$request["oldUserHash"];
        $phoneNumberId = @$request["phone_number_id"];
        
        $updateData = ["uuid" => mysql_real_escape_string($userHash)];
        if ($phoneNumberId) {
            $updateData["whatsappPhoneId"] = mysql_real_escape_string($phoneNumberId);
        }

        $resultUpdate = CocoDB::updateRecords("cocochats",$updateData,"num=".$chatNum." and uuid = '".mysql_real_escape_string($oldUserHash)."'",null,["prefix" => "","ignoreSchema" => true]);
        $resultUpdateMessages = mysql_query("UPDATE cocochats_mensajes SET user = '".mysql_real_escape_string($userHash)."', leido_por = REPLACE(leido_por, '".mysql_real_escape_string($oldUserHash)."', '".mysql_real_escape_string($userHash)."') WHERE chat_id=".$chatNum." and user = '".mysql_real_escape_string($oldUserHash)."'");        

        API::success(["result" => $resultUpdate, "chat_num" => $chatNum, "user_hash" => $userHash, "phone_number_id" => $phoneNumberId]);
    }
