...
class CmsCRUD {
    static function listRecordsBulk($request,$throwError = true){
        ...
        foreach($request as $key => $value){
            $data[$key] = self::listRecords(@$value,$key,$throwError,@$value["order"],@$value["limit"]);
        }
        return $data;
    }
    static function listRecords($whereArray = null,$tableName = null,$throwError = true,$order = null,$limit = null,$group = null){
        global $TABLE_PREFIX;
        if(!$tableName) throw new ApiError("No tableName specified");
        $where = "num!=0";        
        
        if ($whereArray && is_array($whereArray) && @$whereArray["where"]){
        ...
        $groupString = $group ? " GROUP BY ".$group : ''; 
        
        $listRecords_query = mysql_query("SELECT * FROM ".$TABLE_PREFIX.$tableName." WHERE ".$where.$groupString.$orderString.$limitString);
        if(!$listRecords_query) throw new ApiError(mysql_error());

        $listRecords = [];

        $fileName = __DIR__."/../cache/uploads-".date("Y-m-d",time()).".json";
        if (!file_exists($fileName) || true){
            $uploads = mysql_query_fetch_all_assoc("SELECT * FROM ".$TABLE_PREFIX."uploads ORDER BY `order` ASC");
            $uploadsResult = [];
        ...
        while ($row = mysql_fetch_assoc($listRecords_query)) {
            foreach ($possible_keys_of_uploads as $fieldName) {
                if(@$uploadsResult[$tableName][$fieldName][$row['num']]) {
                    $row[$fieldName] = $uploadsResult[$tableName][$fieldName][$row['num']];
                }
            }
            // if (@$row["num"]){
            //     if ($uploadsResult){
            //         foreach ($uploadsResult as $fieldName => $fieldRecords) {
            //             if($fieldName !== $tableName) continue;
            //             foreach ($row as $fieldName => $field) {
            //                 # code...
            //             }
            //             // $row[$fieldName] = isset($fieldRecords[$row['num']]) ? $fieldRecords[$row['num']] : [];
            //             $row[$fieldName] = $fieldRecords;
            //             die(var_dump($row));
            //         }
            //     }
            // }
