...
if (@$_REQUEST["exportCSVPlugin"]){
    ...
    if (!@$campos || !@$seleccion || @$tabla==""){
        ...
        foreach ($data as $i => $row) {
            // Añadido para poder procesar la fila por otro plugin en caso de que un cliente pida algo adicional que no tenga el sistema por defecto
            $reference = ['row' => $row, 'campos' => $campos];
            addPlugins("csv_plugin__pre_each_line", $reference);
            $row = $reference['row'];
