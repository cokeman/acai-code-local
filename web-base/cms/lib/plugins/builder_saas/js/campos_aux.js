            ...
            if (fieldType=="multi"){
                ...
                switch(fieldType){
                    ...
                    case "wysiwyg":
                        ...
                        inputElement.classList.add('ckeditor2');
                    ...
                    case 'uploadV2':
                        ...
                        var inputElement = document.createElement("div");
                        inputElement.classList.add("vue-upload","border","border-gray-400","p-2","bg-gray-300","mb-8");
                        inputElement.id = this.getID();
                        ...
                        buttonIframe.addEventListener("click",function(){
                            ...
                            var recordNum = this.getAttribute("data-recordNum");
                            if (Boolean(recordNum)){
                                _openModal(`/lib/menus/modals/cutefile/index.php?menu=${this.getAttribute("data-table")}&fieldName=${this.getAttribute("data-fieldName")}&recordNum=${this.getAttribute("data-recordNum")}&action=gallery_plugin&callbackFunc=closeGalleryBuilder("${inputElement.id}")`);
                            }else{
                                _openModal(`/lib/menus/modals/cutefile/index.php?menu=${this.getAttribute("data-table")}&fieldName=${this.getAttribute("data-fieldName")}&preSaveTempId=${this.getAttribute("data-preSaveTempId")}&action=gallery_plugin&callbackFunc=closeGalleryBuilder("${inputElement.id}")`);
                            }
                        });
...
//                            break;
    ...
    uploadsV2Init(el,field,data){
        this.postDOMfields[el.id].vueInstance = new Vue({
            ...
            template:`
                <ul class="bg-gray-100 rounded vue-uploads-wrapper overflow-auto mb-8">
            ...
            methods:{
                ...
                async reloadData(force){
                    const where = `tableName='${this.record.tableName}' and fieldName='${this.record.fieldName}' and recordNum=${this.record.recordNum}`;
    ...
    initializeAllPlugins(){
        if (this.postDOMfields){
            for (const uploadFieldIndex in this.postDOMfields){
                ...
                switch(field.type){
                    case "wysiwyg":
                        var editor = CKEDITOR.replace(nodeId);
                        editor.on( 'change', ( evt ) => {
                            nodeId.innerHTML = evt.editor.getData();
                            nodeId.dispatchEvent(new Event("change"));
                            needToSave();
                        });
                    break;
                    case "uploadV2":
                        this.uploadsV2Init(nodeId,field,data)
                    break;
                }
            }
        }
